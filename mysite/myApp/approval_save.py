...
def save(request):
    ...
    for i in range(0, int(proposal_data_length) + 1):
        print(i)
        oppty_num = None;  productno = None; approval_quantity = None; 
        ...
        if productno == '' : 
            productno = None
        buy_place = request.POST.get('buy_place' + str(i),'')
        if buy_place == '' : 
            buy_place = None
        decision_quantity = request.POST.get('decision_quantity' + str(i),'')
        ...
        if approval_unit == '' : 
            approval_unit = 0
        delivery_request_date = request.POST.get('delivery_request_date' + str(i),0)
        ...
        if quote_num != '' and productno != '':
            ...
            data_count = Approval.objects.filter(quote_num=quote_num, productno=productno).count()
            if data_count != 0 :
                data = Approval.objects.get(quote_num=quote_num, productno=productno)
                data.delete()
            Approval.objects.create(quote_num=quote_num, oppty_num=oppty_num, productno=productno, buy_place=buy_place, decision_quantity=decision_quantity,approval_quantity=approval_quantity, approval_unit=approval_unit,
                                    approval_price=approval_price, delivery_request_date=delivery_request_date, approval_balance=approval_balance,total_approval_balance=total_approval_balance )
